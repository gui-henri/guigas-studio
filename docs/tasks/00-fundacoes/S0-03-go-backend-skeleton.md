...
- [ ] `go build ./...` e `go test ./...` verdes dentro de `backend/`
- [ ] `go run ./cmd/api` sobe; `curl localhost:8080/healthz` retorna o JSON de status
- [ ] Logs estruturados (JSON) no stderr, nível controlado por `LOG_LEVEL`
- [ ] SIGINT/SIGTERM encerra sem timeout forçado (shutdown graceful observado no log)
...
- **Owner do módulo**: confirmar o owner real do repo GitHub antes do `go mod init`; renomear
